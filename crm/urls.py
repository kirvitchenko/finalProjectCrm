...
from crm.views.meeting import MeetingListView, MeetingCreateView, MeetingRetrieveView, MeetingAddUserView, \
    MeetingCancelView
from crm.views.tasks import TaskListView, TaskCreateView, TaskRetrieveView, TaskUpdateView, TaskDeleteView, \
    TaskEvaluationView, CommentCreateView, TaskDoneView
...
urlpatterns = [
    ...
    path('tasks/<int:task_pk>/evaluate>', TaskEvaluationView.as_view(), name='task_evaluate'),
    path('tasks/<int:task_pk>/comment/', CommentCreateView.as_view(), name='comment_create'),
    ...
    path('meetings/<int:meeting_pk>/', MeetingRetrieveView.as_view(), name='meeting_retrieve'),
    path('meetings/<int:meeting_pk>/add-user/', MeetingAddUserView.as_view(), name='meeting_add_user'),
    path('meetings/<int:meeting_pk>/cancel/', MeetingCancelView.as_view(), name='meeting_cancel'),

    path('calendar/', CalendarView.as_view(), name='calendar'),  # 1
    path('calendar/<int:year>/<int:month>/', CalendarView.as_view(), name='calendar_month'),  # 2
    path('calendar/<int:year>/<int:month>/<int:day>/', CalendarView.as_view(), name='calendar_day'),
] # 3
